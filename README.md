...
```bash
# 投标询价
python3 bid_inquiry.py \
  -i examples/bid_project.csv \
  -p "XX单位安防系统建设项目" \
  -o output/bid \
  -t 0.13 \
  -w "3年" \
...
|------|------|
| `bid_quote.md` | 分项报价清单 |
| `bid_deviation.md` | 技术偏离表 |
| `bid_full.md` | 完整投标文件 |
| `bid_quote.xlsx` | Excel报价清单 |

### 报价清单格式

| 序号 | 设备名称 | 品牌 | 型号规格 | 单位 | 数量 | 单价 | 合价 | 税率 | 交货期 | 质保 | 偏离 |
|------|----------|------|----------|------|------|------|------|------|--------|------|------|
| 1 | 网络摄像机 | 海康威视 | DS-2CD3T86 | 套 | 20 | ¥470 | ¥9,400 | 13% | 30天 | 3年 | 无偏离 |
